...
defoptions.noise_range = [0.2,0.5];    % frequency range over which to estimate the noise
...
defoptions.fudge_factor = 0.99;            % fudge factor for time constants
...
%         end
    ...
    function g = estimate_time_constants(y,p,sn,lags)
        ...
        if ~isempty(which('xcov')) %signal processing toolbox
            ...
            fprintf('Selmaan Needs to Update time constant code \n'),
            for sBin = 1:floor(length(y)/1e4)
                xc(sBin,:) = xcov(y((sBin-1)*1e4+1:sBin*1e4),lags,'biased');
            end
            xc = median(xc);
        else
            ynormed = (y - mean(y));
